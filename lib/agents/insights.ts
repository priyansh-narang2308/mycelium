...
export async function generateInsight(activities: Activity[], dailyBudget: number): Promise<string> {
  const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
  
  const todayEmissions = activities.reduce((sum, a) => sum + a.co2e, 0);
  const percentage = Math.round((todayEmissions / dailyBudget) * 100);
